...
session = web.session.Session(app, store, initializer={'login': 0,'privilege': 0,'user':'anonymous', 'instanceSelected': 'none',  'instanceList': [], 'lastVisted': '/ThisIsNotTheDataYouAreLookingFor', 'customID': ''})
...
class custom:
	def GET(self):
		...
		if logged():
			...
			render = web.template.render('/var/www/RTP/templates/') 
			return render.login("warning", "You must login before view document data.", _version, )
